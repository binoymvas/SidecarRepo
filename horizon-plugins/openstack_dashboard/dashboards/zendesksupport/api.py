...
class Zendesk:
    ...
    def __init__(self, request):
        ...
        try:
            if keystone_auth_version == 'v3':
                self._manageV3(request)
            else:
                self._manageV2(request)
    ...
    def _get_zendesk_user_id(self):
        ...
        if users.count != 1:
            ...
            new_user = User(name=self._request.name, email=email, password=self._random_pass()) 
    ...
    def create_ticket(self, data):
        ...
        if not self._user_has_email():
            ...
            raise ZendeskError(403, "No email address found for the user")
        try:
            zendesk_user_id = self._get_zendesk_user_id()
    ...
    def create_comment(self, ticket_id, desc):
        """
    ...
    def _ago_calculate(self, source_obj, timezone):
        ...
        if year_diff >= 1:
            ...
            if hour_diff == 1:
                return "1 hour ago"
            return str(hour_diff) + " hours ago"
        elif minute_diff >= 1:
           if minute_diff == 1:
               return "1 month ago"
           return str(minute_diff) + " minutes ago"
        else:
           return "Just now"
